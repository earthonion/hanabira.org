...
interface YouTubeComponentProps {
    videoUrl: string;
    onSubtitleUpdate: (japaneseSubtitle: string, englishSubtitle: string, customSubtitle: string) => void;
}

const YouTubeComponent: React.FC<YouTubeComponentProps> = ({ videoUrl, onSubtitleUpdate }) => {
    const [videoId, setVideoId] = useState<string>("");
    const [currentTime, setCurrentTime] = useState<number>(0);
    const [japaneseSubtitles, setJapaneseSubtitles] = useState<Record<number, string>>({});
    const [englishSubtitles, setEnglishSubtitles] = useState<Record<number, string>>({});
    const [customSubtitles, setCustomSubtitles] = useState<Record<number, string>>({});
    ...
    const setSubtitles = (subtitles: Subtitle[], setSubtitlesState: React.Dispatch<React.SetStateAction<Record<number, string>>>) => {
    ...
    const handleOnProgress: ReactPlayerProps['onProgress'] = ({ playedSeconds }) => {
        setCurrentTime(Math.floor(playedSeconds));
        const newJapaneseSubtitle = japaneseSubtitles[Math.floor(playedSeconds)] || "";
        const newEnglishSubtitle = englishSubtitles[Math.floor(playedSeconds)] || "";
        ...
        onSubtitleUpdate(newJapaneseSubtitle, newEnglishSubtitle, newCustomSubtitle);
    };
            ...
            {videoId && (
                <>
                    ...
                    <div className="flex justify-center my-5">
                        <button className="px-4 py-2 bg-purple-600 rounded hover:bg-purple-700" onClick={handlePlayPause}>
                    ...
                    <Tabs>
                        <Tab label="Japanese">
                            <div className="flex justify-center mt-2 text-4xl h-32">
                                <div className="bg-gray-800 bg-opacity-50 p-4 rounded min-h-full">{currentJapaneseSubtitle || "\u00A0"}</div>
                            </div>
                        </Tab>
                        ...
                        <Tab label="English">
                            <div className="flex justify-center mt-2 text-4xl h-32">
                                <div className="bg-gray-800 bg-opacity-50 p-4 rounded min-h-full">{currentEnglishSubtitle || "\u00A0"}</div>
                            </div>
                        </Tab>
