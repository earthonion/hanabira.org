...
const JapaneseTexts = () => {
    ...
    return (
        <div className="p-6">
            <h1 className="text-2xl font-semibold text-gray-800 mb-6 text-center">Add Custom Japanese Text</h1>
            
            
            
            <form onSubmit={handleSubmit} className="space-y-6 bg-white p-6 rounded-md shadow-md mb-12 max-w-4xl">
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-gray-700">Actual Text</label>
                        <textarea
                            name="actualText"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        <label className="block text-sm font-medium text-gray-700">Parent Tag</label>
                        <input
                            type="text"
                            name="p_tag"
                            value={newText.p_tag}
                ...
                <div className="flex justify-end">
                    <button type="submit" className="px-4 py-2 bg-gray-800 text-white rounded-md hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">
                        Add Text
                    </button>
                </div>
            </form>

            <h2 className="text-xl font-semibold mb-4 text-gray-800">Stored Japanese Texts</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6">
                {texts.map((text) => (
                    ...
                    >
                        <div>
                            <h3 className="text-lg font-medium text-gray-900">{text.topic}</h3>
                            <p className="text-sm text-gray-500">ID: {text._id}</p>
                            <p className="text-gray-700 mt-2 max-h-16 overflow-hidden text-ellipsis">{text.actualText}</p>
                            <p className="text-sm text-gray-500 mt-2">
                                Source: <a href={text.sourceLink} className="text-blue-500 underline">{text.sourceLink}</a>
                            </p>
                            <p className="text-sm text-gray-500 mt-1">Parent Tag: {text.p_tag} | Sub Tag: {text.s_tag}</p>
                            <p className="text-sm text-gray-500 mt-1">User ID: {text.userid} | Language: {text.lang}</p>
                        </div>
                        <button
                            onClick={(e) => {
        ...
        <div className="fixed inset-0 bg-gray-800 bg-opacity-75 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-lg w-full">
                ...
                <div className="mb-4 max-h-[50vh] overflow-y-auto">
                    <p className="text-gray-700 whitespace-pre-line">{textData.actualText}</p>
                </div>
                <p className="text-sm text-gray-500">ID: {textData._id}</p>
                <p className="text-sm text-gray-500">
                    Source: <a href={textData.sourceLink} className="text-blue-500 underline">{textData.sourceLink}</a>
                </p>
                <p className="text-sm text-gray-500 mt-2">Parent Tag: {textData.p_tag} | Sub Tag: {textData.s_tag}</p>
                <p className="text-sm text-gray-500 mt-1">User ID: {textData.userid} | Language: {textData.lang}</p>
