...
if len(sys.argv) != 3:
    print("Usage: python3 app_grammar_generate_mp3_files.py <input_file> <lang>")
    sys.exit(1)
...
# translation table between our nomenclature and gtts naming
gtts_lang = {
    'jp': 'ja',
    'th': 'th',
    'kr': 'ko',
...
dir_name = f"artefacts/grammar_{lang}_{date_time}"
...
for grammar in data:
    for example in grammar['examples']:
        try:
            ...
            print(
                f'Converted {example[lang]} to mp3 and saved to {dir_name}/{file_name}'
            )
            # Add a sleep time to avoid overloading the system
            time.sleep(2)
        except Exception as e:
            logging.error(
                f"Error processing example: {example[lang]}, Error: {e}"
            )
            print(f"Error processing example: {example[lang]}")
            examples_failed += 1
