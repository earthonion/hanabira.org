...
# Iterate through each word object
words_processed = 0
words_failed = 0
for word in data:
    try:
        # Extract the file name from the full path of vocabulary_audio
        file_name = os.path.basename(word["vocabulary_audio"])
        ...
        print(
            f'Converted {word["vocabulary_japanese"]} to mp3 and saved to {dir_name}/{file_name}'
        )
        # Add a sleep time to avoid overloading the system
        time.sleep(0.5)
        time.sleep(1.0)   # got rate limited
    except Exception as e:
        logging.error(
            f"Error processing word: {word['vocabulary_japanese']}, Error: {e}"
        )
        print(f"Error processing word: {word['vocabulary_japanese']}")
        words_failed += 1
