...
//const port = 7000; // just for local VM tests
...
app.get("/api/v1/grammars", (req, res) => {
  //res.send('Hello World!')
  // call like to get subset of words, call without params to get all words
  //curl -X GET http://localhost:8000/api/v1/grammars
...
//     if (pTag && sTag) {
//       // find subset of words based on parameters
//       grammars = await Grammar.find({ p_tag: pTag, s_tag: sTag });
//     } else if (pTag && !sTag) {
//       // find subset of words based on p_tag only
//       grammars = await Grammar.find({ p_tag: pTag });
//     } else {
//       // find all words when no parameters provided
//       grammars = await Grammar.find({});
//     }
//
//     console.log('grammars payload: ' + grammars);
//
...
// // Define the new endpoint to fetch grammar titles based on p_tag
// app.get('/api/v1/grammar-titles', async (req, res) => {
//   // gives list of all grammar titles for given JLPT level
//   // curl -X GET 'http://localhost:8000/api/v1/grammar-titles?p_tag=JLPT_N3'
...
//     const grammars = await Grammar.find({ p_tag: pTag }, 'title');

//     const titles = grammars.map(grammar => grammar.title);

//     res.status(200).json({ titles });
//   } catch (err) {
//     console.error(err);
//     res.status(500).json({ error: 'An error occurred while fetching grammar titles.' });
//   }
// });
